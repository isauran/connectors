...
1. **Созданы новые файлы примера в `temporal/examples/sequin-outbox`**:
   ...
   - `README.md` — Подробная документация на английском языке по локальному запуску Postgres, Sequin, Temporal и самого примера.
   - `README.RU.md` — Локализованная русская версия документации по запуску.
